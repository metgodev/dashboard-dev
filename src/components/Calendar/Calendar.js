...
export default function Calendar({ type, warp, disableHelpers, events }) {
  ...
  const getEvents = useCallback(() => {
    ...
    if (events.loading === false) {
      events.data.map((event) => {
        ...
        if (
          eventsStartDate.getMonth() === today.getMonth() &&
          eventsStartDate.getDate() === today.getDate() &&
          eventsStartDate.getFullYear() === today.getFullYear()
        ) {
  ...
  const doesDayHaveEvents = (events, day) => {
    ...
    events.data.forEach((event) => {
      const eventsStartDate = new Date(event.startDate);
      if (
        eventsStartDate.getDate() === day.getDate() &&
        eventsStartDate.getMonth() === day.getMonth() &&
        eventsStartDate.getYear() === day.getYear()
      ) {
        flag = true;
        return;
      }
      ...
      {type === 2 && (
        <MuiPickersUtilsProvider locale={calendarLang()} utils={DateFnsUtils}>
          <KeyboardDatePicker
            ...
            helperText={
              !disableHelpers &&
              (validDate ? term("date_error") : term("date_format"))
            }
          />
        </MuiPickersUtilsProvider>
      )}
